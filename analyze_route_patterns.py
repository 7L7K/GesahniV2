...
def analyze_routes():
    ...
    try:
        ...
        route_patterns = [
            (
                r'@router\.post\s*\(\s*"/auth/login"',
                "POST",
                "/v1/auth/login",
                "login_v1",
            ),
            (
                r'@router\.post\s*\(\s*"/auth/register"',
                "POST",
                "/v1/auth/register",
                "register_v1",
            ),
            (
                r'@router\.post\s*\(\s*"/auth/logout"',
                "POST",
                "/v1/auth/logout",
                "logout",
            ),
            (
                r'@router\.post\s*\(\s*"/auth/refresh"',
                "POST",
                "/v1/auth/refresh",
                "refresh",
            ),
    ...
    for pattern_name, regex in patterns.items():
        ...
        for path in sorted(path_groups.keys()):
            ...
            for route in route_group:
                ...
                print(
                    f"{duplicate_marker}{schema_marker}{redirect_marker} {route['method']:>6} {path}"
                )
    ...
    compat_routes = [
        r for r in route_info if "compat" in r["handler"] or r["is_redirect"]
    ]
    canonical_routes = [
        r for r in route_info if not r["is_redirect"] and "compat" not in r["handler"]
    ]
    ...
    if redirect_count == len(compat_routes):
        ...
        print(
            f"   ⚠️  {len(compat_routes) - redirect_count} compatibility routes are not redirects"
        )
    ...
    if hidden_count == len(compat_routes):
        ...
        print(
            f"   ⚠️  {len(compat_routes) - hidden_count} compatibility routes are visible in schema"
        )

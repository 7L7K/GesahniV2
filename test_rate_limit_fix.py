...
async def test_whoami_rate_limiting():
    ...
    async with aiohttp.ClientSession() as session:
        ...
        try:
            async with session.get(f"{base_url}/v1/whoami", headers=headers) as response:
            ...
            try:
                async with session.get(f"{base_url}/v1/whoami", headers=headers) as response:
                    if response.status == 200:
                        ...
                        print(f"   Call {i+1}: Rate limited (429) - Retry-After: {retry_after}")
                    else:
...
async def test_health_endpoint():
    ...
    async with aiohttp.ClientSession() as session:
        try:
            async with session.get(f"{base_url}/healthz/ready", headers=headers) as response:
                print(f"   Status: {response.status}")
                if response.status == 200:
                    data = await response.json()
                    print(f"   Response: {json.dumps(data, indent=2)}")

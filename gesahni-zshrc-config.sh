...
# Auto-completion for Gesahni commands
_gesahni_commands() {
    local commands=(
        "start:Start both backend and frontend"
        "stop:Stop all development processes"
        "restart:Restart both services"
        "clear:Clear cookies and restart fresh"
        "back:Start only backend"
        "front:Start only frontend"
        "status:Check if services are running"
        "test:Test localhost configuration"
...
compdef _gesahni_commands gesahni-start gesahni-stop gesahni-restart gesahni-clear gesahni-back gesahni-front gesahni-status gesahni-test gesahni-open gesahni-help

echo "🚀 Gesahni development commands loaded!"
echo "💡 Type 'gesahni-help' or 'gh' for available commands"

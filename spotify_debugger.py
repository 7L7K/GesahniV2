...
class SpotifyDebugger:
    ...
    def log_event(self, event: SpotifyDebugEvent):
        ...
        self.logger.info(
            f"SPOTIFY_EVENT: {event.operation} | {event.status} | {event.user_id}"
        )
        ...
        for event in self.events:
            ...
            if event.duration_ms:
                ...
                stats[op]["avg_duration"] = (
                    current_avg * (count - 1) + event.duration_ms
                ) / count
...
def debug_log_spotify_request(operation: str, user_id: str = "unknown", **metadata):
    ...
    event = SpotifyDebugEvent(
        operation=operation, user_id=user_id, status="request", metadata=metadata
    )
    spotify_debugger.log_event(event)


def debug_log_spotify_response(
    operation: str,
    user_id: str = "unknown",
    status: str = "success",
    error: str = None,
...
async def debug_spotify_integration():
    ...
    if recent_events:
        for event in recent_events:
            status_icon = (
                "✅"
                if event["status"] == "success"
                else "❌" if event["status"] == "error" else "🔄"
            )
            duration = (
                f" ({event['duration_ms']:.1f}ms)" if event["duration_ms"] else ""
            )
            print(
                f"  {status_icon} {event['operation']} | {event['user_id']} | {event['status']}{duration}"
            )
            if event["error"]:
    ...
    if stats:
        for operation, data in stats.items():
            ...
            print(
                f"  {operation}: {data['count']} total | {success_rate:.1f}% success | {data['avg_duration']:.1f}ms avg"
            )
    else:

...
def test_idempotency():
    ...
    headers1 = {"content-type": "application/json", "Idempotency-Key": "abc123"}
    data1 = {"prompt": "hi"}

    try:
        response1 = requests.post(
            f"{base_url}/v1/ask", headers=headers1, json=data1, timeout=10
        )
    ...
    headers2 = {"content-type": "application/json", "Idempotency-Key": "abc123"}
    data2 = {"prompt": "hi"}

    try:
        response2 = requests.post(
            f"{base_url}/v1/ask", headers=headers2, json=data2, timeout=10
        )
    ...
    if (
        ...
        and response1.text == response2.text
    ):
    ...
    headers3 = {"content-type": "application/json", "Idempotency-Key": "xyz789"}
    data3 = {"prompt": "hi"}

    try:
        response3 = requests.post(
            f"{base_url}/v1/ask", headers=headers3, json=data3, timeout=10
        )

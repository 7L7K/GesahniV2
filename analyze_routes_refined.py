...
def main():
    ...
    if not results["missing_tests"]:
        ...
        admin_routes = {
            k: v for k, v in results["missing_tests"].items() if "/admin" in k
        }
        auth_routes = {
            k: v
            for k, v in results["missing_tests"].items()
            if "/auth" in k and "/admin" not in k
        }
        api_routes = {
            k: v for k, v in results["missing_tests"].items() if k.startswith("/v1/")
        }
        other_routes = {
            k: v
            for k, v in results["missing_tests"].items()
    ...
    if not results["method_mismatches"]:
        ...
        for path, info in sorted(results["method_mismatches"].items()):
            print(f"🚨 {path}")
            print(
                f"   Tested methods not defined: {', '.join(sorted(info['tested_but_not_defined']))}"
            )
            print(f"   Defined methods: {', '.join(sorted(info['defined_methods']))}")
            print(
                "   Explanation: Tests are calling methods that the route doesn't support."
            )
    print("\n" + "=" * 80)

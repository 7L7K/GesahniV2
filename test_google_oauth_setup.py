...
def test_google_oauth_config():
    ...
    print(f"📋 GOOGLE_CLIENT_ID: {'✅ Set' if client_id and client_id != 'YOUR_ACTUAL_GOOGLE_CLIENT_ID_HERE' else '❌ Not configured'}")
    print(f"🔐 GOOGLE_CLIENT_SECRET: {'✅ Set' if client_secret and client_secret != 'YOUR_ACTUAL_GOOGLE_CLIENT_SECRET_HERE' else '❌ Not configured'}")
    ...
    if not client_id or client_id == 'YOUR_ACTUAL_GOOGLE_CLIENT_ID_HERE':
        ...
        print("   See: https://console.cloud.google.com/ -> APIs & Services -> Credentials")
        return False

    # Test login URL endpoint
    try:
        response = requests.get("http://localhost:8000/v1/google/auth/login_url", timeout=5)

...
def analyze_routes():
        ...
        if isinstance(route, APIRoute):
            ...
            for name, param in sig.parameters.items():
                ...
                if hasattr(param, "default") and hasattr(param.default, "__name__"):
                    ...
                    if any(
                        keyword in dep_name
                        for keyword in ["auth", "token", "user", "current"]
                    ):
                        route_info["auth_deps"].append(
                            f"{name}: {param.default.__name__}"
                        )
            ...
            if hasattr(route, "response_model") and route.response_model:
                ...
                and "return" in route.endpoint.__annotations__
            ):
                route_info["response_model"] = str(
                    route.endpoint.__annotations__["return"]
                )
    ...
    for path, methods in path_only.items():
        if len(methods) > len(set(methods)):
            issues["shadowed_routes"].append({"path": path, "methods": methods})
    ...
    categories = {
        ...
        "API": lambda r: r["path"].startswith("/v1/")
        and not any(
            x in r["path"] for x in ["admin", "auth", "me", "whoami", "sessions"]
        ),
    ...
    for category_name, category_filter in categories.items():
        ...
        if category_routes:
            ...
            for route in category_routes[:3]:  # Show first 3 per category
                ...
                print(
                    f'  Handler: {route["handler"]} ({route["module"].split(".")[-1]})'
                )
                print(
                    f'  Auth: {"✅" if route["auth_deps"] else "❌"} {route["auth_deps"] or ["None"]}'
                )

...
class OAuthEndToEndTest:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.log_test_result(
                    "Server Health Check", True, "Server responding correctly"
                )
                return True
            else:
                self.log_test_result(
            ...
            self.log_test_result(
                "Server Health Check", False, f"Server not responding: {e}"
            )
            return False
    ...
    def test_oauth_login_url_generation(self):
        ...
        try:
            response = requests.get(
                f"{self.base_url}/v1/auth/google/login_url", timeout=5
            )
            if response.status_code != 200:
                self.log_test_result(
                    "OAuth Login URL", False, f"HTTP {response.status_code}"
                )
                return False
            ...
            if missing_params:
                self.log_test_result(
                    "OAuth Login URL", False, f"Missing parameters: {missing_params}"
                )
                return False
            ...
            if "openid" not in scope:
                self.log_test_result(
                    "OAuth Login URL", False, f"Missing openid scope in: {scope}"
                )
                return False
            ...
            if not state:
                self.log_test_result(
                    "OAuth Login URL", False, "Missing state parameter"
                )
                return False

            self.log_test_result(
                "OAuth Login URL", True, "Generated correctly with openid scope"
            )
            return True
    ...
    def test_oauth_callback_validation(self):
        ...
        try:
            ...
            response = requests.get(
                f"{self.base_url}/v1/auth/google/callback", timeout=5
            )
            if response.status_code != 400:
                self.log_test_result(
                    "Callback Validation", False, "Should reject missing code parameter"
                )
                return False

            # Test missing state parameter
            response = requests.get(
                f"{self.base_url}/v1/auth/google/callback?code=test_code", timeout=5
            )
            if response.status_code != 400:
                self.log_test_result(
                    "Callback Validation",
            ...
            self.log_test_result(
                "Callback Validation", True, "Properly validates required parameters"
            )
            return True
    ...
    def test_id_token_processing_logic(self):
        ...
        try:
            ...
            if decoded.get("iss") != "https://accounts.google.com":
                self.log_test_result(
                    "ID Token Processing", False, "Failed to extract issuer"
                )
                return False

            if decoded.get("sub") != "123456789012345678901":
                self.log_test_result(
                    "ID Token Processing", False, "Failed to extract subject"
                )
                return False

            self.log_test_result(
                "ID Token Processing", True, "Successfully processes JWT tokens"
            )
            return True
    ...
    def test_third_party_token_creation(self):
        ...
        try:
            ...
            if not hasattr(token, "id_token"):
                self.log_test_result(
                    "ThirdPartyToken Creation", False, "Cannot add id_token attribute"
                )
                return False

            if token.id_token != mock_id_token:
                self.log_test_result(
                    "ThirdPartyToken Creation",
                    False,
                    "id_token not preserved correctly",
                )
                return False
            ...
            self.log_test_result(
                "ThirdPartyToken Creation",
                True,
                "Can preserve id_token for callback processing",
            )
            return True
        ...
        try:
            ...
            if not has_db:
                self.log_test_result(
                    "Database Connection",
                    True,
                    "Database not required for OAuth (optional)",
                )
                return True
            ...
            if result and result[0] == 1:
                self.log_test_result(
                    "Database Connection", True, "Database is accessible"
                )
                return True
            else:
                self.log_test_result(
                    "Database Connection", False, "Database query failed"
                )
                return False
    ...
    def test_google_scopes_configuration(self):
        ...
        try:
            ...
            if "openid" not in scopes:
                self.log_test_result(
                    "Google Scopes", False, "Missing required openid scope"
                )
                return False
            ...
            if missing_scopes:
                self.log_test_result(
                    "Google Scopes",
                    False,
                    f"Missing recommended scopes: {missing_scopes}",
                )
                return False

            self.log_test_result(
                "Google Scopes", True, f"Configured correctly: {len(scopes)} scopes"
            )
            return True
    ...
    def test_environment_variables(self):
        ...
        if missing_vars:
            self.log_test_result(
                "Environment Variables", False, f"Missing: {missing_vars}"
            )
            return False
        ...
        if not client_id.endswith(".apps.googleusercontent.com"):
            self.log_test_result(
                ...
                False,
                "Client ID doesn't look like Google OAuth ID",
            )
            return False
        ...
        try:
            ...
            if response.status_code == 401:
                self.log_test_result(
                    ...
                    True,
                    "/v1/admin/users/me endpoint properly secured",
                )
                return True
            else:
                self.log_test_result(
                    "Frontend API Endpoints",
                    False,
                    f"/v1/admin/users/me returned {response.status_code}, expected 401",
                )
                return False
    ...
    def print_failed_tests(self):
        ...
        failed_tests = [
            (name, details) for name, passed, details in self.test_results if not passed
        ]
...
def main():
    ...
    if success and tester.server_running:
        ...
        print("\nNext steps:")
        print(
            "1. Start the server: uvicorn app.main:app --host 127.0.0.1 --port 8000 --reload"
        )

...
def test_csrf_bypass():
    ...
    try:
        response = requests.post(f"{BASE_URL}/v1/ask", json=data, headers=headers, timeout=5)
...
def main():
    ...
    results.append(("Authorization Header Logging", test_authorization_header_logging()))
    ...
    print("   4. Check backend logs for 'whoami.header_check has_authorization_header=true'")

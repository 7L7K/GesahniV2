...
class PerfAnalyzer:
    ...
    def save_baseline(
        self, endpoint: str, metrics: Dict[str, float], timestamp: Optional[str] = None
    ) -> None:
        ...
        baseline_file = (
            self.baseline_dir
            / f"{endpoint.replace('/', '_').lstrip('_')}_baseline.json"
        )
    ...
    def analyze_results_directory(
        ...
        for csv_file in csv_files:
            ...
            regression, comparison = self.compare_with_baseline(
                endpoint, metrics, threshold_percent
            )
            ...
            if regression:
                ...
                for metric, data in comparison.items():
                    if isinstance(data, dict) and data.get("regression", False):
                        print(
                            f"   {metric}: {data['percent_change']:.1f}% increase "
...
def main():
    ...
    parser.add_argument(
        "--results-dir", required=True, help="Directory containing CSV results"
    )
    parser.add_argument(
        "--baseline-dir", default="perf_baselines", help="Directory for baseline files"
    )
    parser.add_argument(
        "--save-baselines",
        action="store_true",
    ...
    if args.endpoint:
        ...
        csv_file = (
            ...
            / f"{args.endpoint.replace('/', '_').lstrip('_')}_*.csv"
        )
        matching_files = list(
            Path(args.results_dir).glob(
                f"{args.endpoint.replace('/', '_').lstrip('_')}_*.csv"
            )
        )
        ...
        regression, comparison = analyzer.compare_with_baseline(
            args.endpoint, metrics, args.threshold
        )
        ...
        return analyzer.analyze_results_directory(
            args.results_dir, args.save_baselines, args.threshold
        )

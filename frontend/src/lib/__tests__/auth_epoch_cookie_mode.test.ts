...
// Tests for cookie-mode auth epoch bumps and auth-sensitive cache keys

function withCookieMode<T>(fn: (api: any) => Promise<T> | T): Promise<T> | T {
    // Ensure the module reads cookie-mode before import
    jest.resetModules();
    const prev = process.env.NEXT_PUBLIC_HEADER_AUTH_MODE;
    ...
    try {
        ...
        process.env.NEXT_PUBLIC_HEADER_AUTH_MODE = prev;
    }
}

describe('cookie-mode auth epoch + cache keys', () => {
    beforeEach(() => {
        localStorage.clear();
    ...
    test('login bumps auth epoch in cookie mode', async () => {
        await withCookieMode(async (api) => {
            const { login, getAuthEpoch } = api as { login: Function; getAuthEpoch: () => string };
    ...
    test('register bumps auth epoch in cookie mode', async () => {
        await withCookieMode(async (api) => {
            const { register, getAuthEpoch } = api as { register: Function; getAuthEpoch: () => string };
            expect(getAuthEpoch()).toBe('0');
    ...
    test('epoch segregates short cache across accounts (no stale reuse after login)', async () => {
        await withCookieMode(async (api) => {
            const { apiFetch, login } = api as { apiFetch: Function; login: Function };
            // 1) First GET caches short response under ck:0
    ...
    test('device context prevents short-cache reuse across device_id changes', async () => {
        await withCookieMode(async (api) => {
            const { getMusicState } = api as { getMusicState: () => Promise<{ device_id: string | null }> };
    ...
    test('getTvConfig includes resident_id in query', async () => {
        await withCookieMode(async (api) => {
            const { getTvConfig } = api as { getTvConfig: (residentId: string, token: string) => Promise<any> };
    ...
    test('clearTokens bumps auth epoch again in header mode', async () => {
        // Use default header-auth mode from jest.setup
        // eslint-disable-next-line @typescript-eslint/no-var-requires
        const api = require('@/lib/api') as { setTokens: Function; clearTokens: Function; getAuthEpoch: () => string };
        const { setTokens, clearTokens, getAuthEpoch } = api;
    ...
    test('contextKey affects inflight dedupe: same ctx coalesces, different ctx refetches', async () => {
        // eslint-disable-next-line @typescript-eslint/no-var-requires
        const api = require('@/lib/api') as { apiFetch: Function };
        const { apiFetch } = api;

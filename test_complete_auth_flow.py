    ...
    if whoami_response.status_code == 200:
        ...
        if should_call_refresh:
            ...
            if refresh_response.status_code == 200:
                ...
                print_result("boot_sequence_correct", True, "whoami → refresh (when needed)")
            else:
                print_result("boot_sequence_correct", True, f"whoami → refresh failed as expected: {refresh_response.status_code}")
        else:
            print("2. No refresh call needed (authenticated or no refresh cookie)")
            print_result("boot_sequence_correct", True, "whoami only (no refresh needed)")
            
    else:
        print_result("boot_sequence_correct", False, f"whoami failed: {whoami_response.status_code}")
    ...
    if state_response.status_code == 401:
        ...
        if has_refresh_cookie:
            ...
            if refresh_response.status_code == 200:
                ...
                print(f"   GET /v1/state (retry) - Status: {retry_response.status_code}")
                
                if retry_response.status_code == 200:
                    print_result("401_handling_correct", True, "401 → refresh → retry → success")
                else:
                    print_result("401_handling_correct", True, f"401 → refresh → retry → {retry_response.status_code}")
            else:
                print("   Refresh failed → should settle in logged out state")
                print_result("401_handling_correct", True, "401 → refresh failed → logged out")
        else:
        ...
        print_result("401_handling_correct", False, f"Expected 401, got {state_response.status_code}")
...
def test_cors_vs_auth():
    ...
    if response.status_code == 401:
        ...
        print_result("401_has_cors_headers", has_cors_headers, 
                    f"Origin: {cors_origin}, Credentials: {cors_credentials}")
        
        # Check content type
        content_type = response.headers.get("Content-Type", "")
        print_result("401_returns_json", "application/json" in content_type, 
...
def test_refresh_cookie_only():
    ...
    print_result("refresh_no_authorization_header", not auth_header_present,
    ...
    if response.status_code == 200:
        print_result("refresh_sets_access_cookie", bool(set_cookie and "access_token" in set_cookie),
        ...
        try:
            body = response.json()
            print(f"Response body: {json.dumps(body, indent=2)}")
            print_result("refresh_returns_tokens", "access_token" in body, 
                        f"access_token in response: {'access_token' in body}")
        except:
            print_result("refresh_returns_tokens", False, "Could not parse response as JSON")
    else:
        print_result("refresh_sets_access_cookie", True, f"Refresh failed as expected: {response.status_code}")
...
def test_no_html_redirects():
    ...
    for endpoint in endpoints:
        ...
        try:
            ...
            print_result(f"{endpoint}_no_redirect", not is_redirect, f"Status: {response.status_code}")
            ...
            print_result(f"{endpoint}_no_location_header", not location_header, f"Location: {location_header}")
            
            # Check first 100 chars of body for HTML
            try:
                body_text = response.text[:100]
                has_html = "<html" in body_text.lower() or "<!doctype" in body_text.lower()
                print_result(f"{endpoint}_no_html_body", not has_html, f"Body starts with: {body_text}")
            except:
...
def test_authenticated_flow():
    ...
    if login_response.status_code == 200:
        ...
        print_result("login_sets_cookies", has_access_cookie and has_refresh_cookie,
        ...
        if whoami_response.status_code == 200:
            ...
            print_result("whoami_with_cookies", is_authenticated and source == "cookie",
            ...
            if state_response.status_code == 200:
                ...
                if refresh_response.status_code == 200:
                    print_result("refresh_with_cookies", True, "Refresh successful with cookies")
                    ...
                    print_result("refresh_rotates_cookies", new_access_cookie and new_refresh_cookie,
                                f"New access_token: {new_access_cookie}, new refresh_token: {new_refresh_cookie}")
                else:
                    print_result("refresh_with_cookies", False, f"Refresh failed: {refresh_response.status_code}")
            else:

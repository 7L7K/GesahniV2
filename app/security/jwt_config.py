...
def _require_strong_secret(secret: str, *, allow_dev: bool) -> None:
    ...
    if not allow_dev and re.fullmatch(
        r"(dev|staging|prod|test|secret|token)[-_]?\d*", secret, re.I
    ):
        raise RuntimeError(
...
def get_jwt_config(*, allow_dev_weak=None) -> JWTConfig:
    ...
    algs = [
        a.strip().upper()
        for a in os.getenv("JWT_ALGS", "HS256").split(",")
        if a.strip()
    ]
    if not algs:
        algs = ["HS256"]
    ...
    if alg == "HS256":
        ...
        if not secret and not priv:
            raise RuntimeError(
                "JWT_SECRET missing for HS256 (or use JWT_PRIVATE_KEYS for rotation)."
            )
        if secret and not priv:
            ...
            if secret:
                raise RuntimeError(
                    "Cannot specify both JWT_SECRET and JWT_PRIVATE_KEYS for HS256."
                )
            missing = sorted(set(priv.keys()) - set(pub.keys()))
            if missing:
                raise RuntimeError(
                    f"Public key(s) missing for kid(s): {', '.join(missing)}"
                )
            return JWTConfig(
                alg="HS256",
                secret=None,
            ...
            raise RuntimeError(
                "Invalid HS256 configuration: specify JWT_SECRET or JWT_PRIVATE_KEYS/JWT_PUBLIC_KEYS."
            )

    if alg in ("RS256", "ES256"):
        if not (priv and pub):
            raise RuntimeError(
                f"{alg} requires JWT_PRIVATE_KEYS and JWT_PUBLIC_KEYS JSON (kid->PEM)."
            )
        missing = sorted(set(priv.keys()) - set(pub.keys()))
        if missing:
            raise RuntimeError(

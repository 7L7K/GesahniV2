...
class LocalStorage:
    ...
    def _maybe_cleanup(self) -> None:
        ...
        if expired_keys:
            logger.debug(
                f"Cleaned up {len(expired_keys)} expired entries from local storage"
            )
        ...
        while _cleanup_running:
            try:
                ...
                if total_entries > 1000:  # Log warning if too many entries
                    logger.warning(
                        f"High local storage usage: {total_entries} total entries"
                    )
...
async def is_refresh_allowed(sid: str, jti: str) -> bool:
    ...
    if r is not None:
        try:
            ...
            logger.warning(
                f"Redis is_refresh_allowed failed, falling back to local: {e}"
            )
...
async def claim_refresh_jti(sid: str, jti: str, ttl_seconds: int) -> bool:
    ...
    if r is not None:
        try:
            ...
            logger.warning(
                f"Redis claim_refresh_jti failed, falling back to local: {e}"
            )
...
async def claim_refresh_jti_with_retry(
    sid: str, jti: str, ttl_seconds: int, max_retries: int = 3
) -> tuple[bool, str | None]:
    ...
    if r is not None:
        try:
            ...
            logger.warning(
                f"Redis claim_refresh_jti_with_retry failed, falling back to local: {e}"
            )
...
async def set_last_used_jti(sid: str, jti: str, ttl_seconds: int | None = None) -> None:
    ...
    if r is not None:
        try:
            ...
            logger.warning(
                f"Redis set_last_used_jti failed, falling back to local: {e}"
            )
...
async def get_last_used_jti(sid: str) -> str | None:
    ...
    if r is not None:
        try:
            ...
            logger.warning(
                f"Redis get_last_used_jti failed, falling back to local: {e}"
            )
...
async def is_refresh_family_revoked(sid: str) -> bool:
    ...
    if r is not None:
        try:
            ...
            logger.warning(
                f"Redis is_refresh_family_revoked failed, falling back to local: {e}"
            )
...
async def is_access_revoked(jti: str) -> bool:
    ...
    if r is not None:
        try:
            ...
            logger.warning(
                f"Redis is_access_revoked failed, falling back to local: {e}"
            )
...
async def record_pat_last_used(pat_id: str, ttl_seconds: int | None = None) -> None:
    ...
    if r is not None:
        try:
            ...
            logger.warning(
                f"Redis record_pat_last_used failed, falling back to local: {e}"
            )

...
async def websocket_transcribe(ws: WebSocket, user_id: str = Depends(get_current_user_id), _v: None = dep_verify_ws()):
    from app.api.sessions import websocket_transcribe as _wt
    return await _wt(ws, user_id)

@router.websocket("/ws/storytime")
async def websocket_storytime(ws: WebSocket, user_id: str = Depends(get_current_user_id), _v: None = dep_verify_ws()):
...
async def websocket_http_handler(request: Request, path: str):
    try:
        ...
        record_ws_reconnect_attempt(endpoint=f"/v1/ws/{path}", reason="http_request_to_ws_endpoint", user_id="unknown")
    except Exception:
        pass
    return Response(

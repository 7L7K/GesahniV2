...
@router.get(
    "/users/{user_id}/identities", dependencies=[Depends(require_scope("admin:read"))]
)
...
@router.post(
    "/users/{user_id}/identities/{identity_id}/unlink",
    dependencies=[Depends(require_scope("admin:write"))],
)
async def admin_unlink_identity(
    user_id: str, identity_id: str, force: bool | None = Query(default=False)
):
...
@router.post(
    ...
    responses={
        202: {"model": AdminBootstrapResponse},

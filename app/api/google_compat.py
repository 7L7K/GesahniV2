...
async def compat_google_oauth_callback(request: Request):
    ...
    if legacy_enabled != "1":
        logger.info("google_oauth.legacy_disabled", extra={
            "req_id": getattr(request.state, "req_id", "-"),
            "path": str(request.url.path),
            "component": "google_oauth"
        })
        raise HTTPException(status_code=410, detail="Legacy Google OAuth endpoint disabled")
    ...
    logger.info("google_oauth.legacy_used", extra={
        "req_id": getattr(request.state, "req_id", "-"),
        "path": str(request.url.path),
        "legacy": True,
        "component": "google_oauth"
    ...
    if isinstance(response, Response):
        ...
        response.headers["Link"] = '<https://docs.example.com/deprecated/google-oauth>; rel="sunset"'

...
async def apple_start(request: Request):
    ...
    resp = RedirectResponse(url="/v1/auth/apple/callback?state=" + up.quote(state) + "&code=fake-dev-code", status_code=302)
    ...
    if os.getenv("APPLE_CLIENT_ID") and os.getenv("APPLE_TEAM_ID"):
        ...
        redirect_uri = os.getenv("APPLE_REDIRECT_URI", f"{os.getenv('APP_URL','http://localhost:8000')}/v1/auth/apple/callback")
        scope = "name email"
        auth_params = {
            "client_id": client_id,
        ...
        resp = RedirectResponse(url=APPLE_AUTH_URL + "?" + up.urlencode(auth_params), status_code=302)

    return resp


@router.get("/v1/auth/apple/callback", include_in_schema=False)
async def apple_callback(state: str | None = None, code: str | None = None, request: Request = None):

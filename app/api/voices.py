...
router = APIRouter(
    ...
    dependencies=[
        ...
        Depends(optional_require_scope("music:control")),
    ],
)
...
async def catalog(user_id: str = Depends(get_current_user_id)) -> dict:
    ...
    voices = [
        {
            "id": os.getenv("OPENAI_TTS_VOICE", "alloy"),
            "label": "Expressive (Alloy)",
            "tier": "mini_tts|tts1|tts1_hd",
        },
    ...
    return {
        "openai": voices,
        "piper": [
            {"id": os.getenv("PIPER_VOICE", "en_US-amy-low"), "label": "Local Piper"}
        ],
    }

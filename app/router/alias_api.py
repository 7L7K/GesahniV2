...
def _wrap_path_param_handler(fn_name: str, module: str, param_name: str = "key"):
    ...
    async def _adapter(request: Request):
        ...
        key = request.path_params.get(param_name) or request.query_params.get(param_name)
...
def _wrap_ha_call_service(fn_name: str, module: str):
    ...
    async def _adapter(request: Request):
        ...
        if not domain or not service:
            return JSONResponse({"detail": "missing domain or service"}, status_code=400)
        try:
...
ALIASES.update({
    ...
    "/spotify/status": ("GET", _safe_import("app.api.spotify", "integrations_spotify_status")),
    "/google/status": ("GET", _safe_import("app.api.google", "integrations_google_status")),

    # Home Assistant compatibility
    "/ha/entities": ("GET", _safe_import("app.home_assistant", "get_states")),
    "/ha/service": ("POST", _wrap_ha_call_service("call_service", "app.home_assistant")),
    "/ha/resolve": ("GET", _wrap_query_param_handler("resolve_entity", "app.home_assistant", "name")),
    ...
    "/music/devices": ("GET", _safe_import("app.router.music_api", "music_devices")),
    "/music/device": ("PUT", _safe_import("app.router.music_api", "set_music_device")),

    # Transcribe / TTS / Admin
    "/transcribe/{job_id}": ("POST", _safe_import("app.router.transcribe_api", "transcribe_job")),
    "/tts/speak": ("POST", _safe_import("app.router.tts_api", "tts_speak")),
    "/admin/reload_env": ("POST", _safe_import("app.router.admin_extra_api", "reload_env")),
    "/admin/self_review": ("POST", _safe_import("app.router.admin_extra_api", "self_review")),
    "/admin/vector_store/bootstrap": ("POST", _safe_import("app.router.admin_extra_api", "bootstrap_vector_store")),
    ...
    "/admin/metrics": ("GET", _safe_import("app.router.admin_api", "admin_metrics")),
    "/admin/ping": ("GET", _safe_import("app.router.admin_api", "admin_ping")),
    "/admin/rate-limits/{key}": ("GET", _wrap_path_param_handler("get_rate_limit_stats", "app.auth", "key")),
    "/admin/rbac/info": ("GET", _safe_import("app.router.admin_api", "admin_rbac_info")),
    "/admin/router/decisions": ("GET", _safe_import("app.router.admin_api", "admin_router_decisions")),
    "/admin/system/status": ("GET", _safe_import("app.router.admin_api", "admin_system_status")),
    "/admin/tokens/google": ("GET", _safe_import("app.router.admin_api", "admin_google_tokens")),

    # Integrations namespace parity
    "/integrations/google/status": ("GET", _safe_import("app.api.google", "integrations_google_status")),
    "/integrations/spotify/status": ("GET", _safe_import("app.api.spotify", "integrations_spotify_status")),
...
async def _fallback(request: Request, path: str, method: str):
    ...
    if path in ("/whoami", "/me"):
        return JSONResponse({"user": None, "authenticated": False, "detail": "not_authenticated"}, status_code=401)
    ...
    if path.endswith("/status") or path.endswith("/status"):
        ...
        return JSONResponse({"connected": False, "detail": "not_configured"}, status_code=200)

    # Calendar endpoints -> 200 with empty lists
    if path in ("/list", "/next", "/today") or path.startswith("/calendar/") or path.startswith("/calendar"):
        if path.endswith("/next") or path == "/next":
            # Normalized shape for /calendar/next
            return JSONResponse({"event": None, "detail": "no_upcoming_events"}, status_code=200)
    ...
    if path == "/music":
        ...
        return JSONResponse({"playing": False, "device": None, "track": None}, status_code=200)

...
try:
    ...
    AUTH_LEGACY_HITS_TOTAL = Counter(
        ...
        "Total hits on legacy auth endpoints",
        ["endpoint"]
    )
except ImportError:
...
class LegacyAlias:
    ...
    def __init__(self, path: str, successor: str, method: str, handler: Callable[..., Any]):
        self.path = path
    ...
    async def __call__(self, *args, **kwargs):
        ...
        if hasattr(result, 'headers'):
            ...
            result.headers.setdefault("Link", f'<{self.successor}>; rel="successor-version"')

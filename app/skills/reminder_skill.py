...
from .parsers import parse_when
    ...
    async def run(self, prompt: str, match: re.Match) -> str:
        ...
        if gd.get("when"):
            ...
            if isinstance(parsed, datetime):
                ...
                # set skill_why
                self.skill_why = f"reminder.set: when={run_dt.isoformat()}"
                await record_action(
                    "reminder.set",
                    idempotency_key=idemp,
                    metadata={"when": run_dt.isoformat(), "task": task},
                    reversible=True,
                )
        ...
        if gd.get("period"):
            ...
            if period in {"day", "week", "month"}:
                ...
                await record_action(
                    "reminder.set",
                    idempotency_key=idemp,
                    metadata={"period": period, "task": task},
                    reversible=True,
                )
                ...
                await record_action(
                    "reminder.set",
                    idempotency_key=idemp,
                    metadata={"day_of_week": period[:3], "task": task},
                    reversible=True,
                )
                return (

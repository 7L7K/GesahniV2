...
class CheckinSkill(Skill):
    PATTERNS = [
        re.compile(r"check in( now)?(?: to (?P<topic>.+))?", re.I),
        re.compile(r"reschedule (?P<mins>\d+) minutes", re.I),
    ]
    ...
    async def run(self, prompt: str, match) -> str:
        ...
        if gd.get("mins"):
            ...
            await record_action(
                "checkin.reschedule",
                idempotency_key=f"checkin:resched:{int(datetime.now().timestamp())}",
                metadata={"mins": mins},
                reversible=False,
            )
        ...
        await record_action(
            "checkin.note",
            idempotency_key=f"checkin:note:{int(datetime.now().timestamp())}",
...
class CheckinSkill(Skill):
    PATTERNS = [
        re.compile(
            r"schedule a check-?in with (?P<person>[\w\s]+) (?:at|on) (?P<time>.+)",
            re.I,
        ),
    ...
    async def run(self, prompt: str, match: re.Match) -> str:
        ...
        await record_action(
            "checkin.schedule",
            idempotency_key=f"checkin:{person}:{int(datetime.now().timestamp()//10)}",

...
# Internal Auth legacy redirects (more specific than generic LEGACY_HITS)
INTERNAL_AUTH_LEGACY_HITS = Counter(
    "internal_auth_legacy_hits_total",
    "Internal auth legacy endpoint hits",
    ["endpoint"]
)
...
GOOGLE_STATUS_REQUESTS_COUNT = Counter(
    "google_status_requests_count",
    "Google status API requests",
    ["status", "auth_state"],
)

GOOGLE_STATUS_CONNECTED = Counter(
    "google_status_connected_total",
    "Google status responses reporting connected",
    ["user"],
)
...
# Google OAuth error metrics
try:
    GOOGLE_OAUTH_ERRORS_TOTAL = Counter(
        "google_oauth_errors_total",
        "Total number of Google OAuth errors by type",
        ["error_type"],
    )
except Exception:  # pragma: no cover
    ...
    GOOGLE_OAUTH_ERRORS_TOTAL = _GOAuthErrors()  # type: ignore

# Spotify OAuth error metrics
try:
    SPOTIFY_OAUTH_ERRORS_TOTAL = Counter(
        "spotify_oauth_errors_total",
        "Total number of Spotify OAuth errors by type",
        ["error_type"],
    )
except Exception:  # pragma: no cover
    ...
    SPOTIFY_OAUTH_ERRORS_TOTAL = _SOAuthErrors()  # type: ignore

...
class MemGPT:
    """Simple in-process memory manager (governed variant).

    Stores prompt/answer pairs per session and supports governed claim writes.
    """
    ...
    def __init__(
        ...
        self.storage_path = Path(
            storage_path
            or Path(__file__).resolve().parent.parent.parent / "data" / "memories.json"
        )
    ...
    # Public API
    # ------------------------------------------------------------------
    # Removed lightweight write_claim overload to avoid shadowing governed version below
    def write_claim(
        self,
        *,
        session_id: str,
        ...
        now = time.time()
        # Enforce per-session write quota
        max_writes = int(os.getenv("MEM_WRITE_QUOTA_PER_SESSION", "30"))
        if not pinned and max_writes > 0:
            with self._lock:
                current = len(self._data.get(session_id, []))
    ...
    def store_interaction(
        ...
        *,
        ...
        with self._lock:
            ...
            if is_pinned:
                # Require elevated scope when pinning in API layer; here only mark auditable flag
            ...
            bucket.append(
                {
                    ...
                    "timestamp": now,
                    "hash": entry_hash,
                    "audit": {"pinned": bool(is_pinned), "user_id": user_id},
                }
            )
    ...
    def _novelty_score(self, text: str, user_id: Optional[str]) -> float:
        corpus: List[str] = []
    ...
    def _is_duplicate(self, *, checksum: str, simhash: int, text: str, user_id: Optional[str]) -> bool:

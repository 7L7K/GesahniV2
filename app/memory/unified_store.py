    ...
    try:
        if config.scheme == "memory":
            ...
            try:
                ...
                raise RuntimeError("ChromaVectorStore unavailable (chromadb not installed)")
            ...
            if config.host or config.port:
                ...
                logger.info("Using ChromaVectorStore (Cloud): %s.%s", config.username, config.password)
            ...
            if not nested_dsn:
                raise ValueError("Dual mode requires nested DSN (e.g., dual://qdrant://host:port)")
            ...
            os.environ["VECTOR_DUAL_QA_WRITE_BOTH"] = config.get_param("qa_write_both", "0")

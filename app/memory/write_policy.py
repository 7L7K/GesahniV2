...
class MemoryWritePolicy:
    ...
    def __init__(self):
        ...
        self.min_profile_response_length = int(os.getenv("PROFILE_MIN_RESPONSE_LENGTH", "20"))
        ...
        self.enable_memory_policy = os.getenv("ENABLE_MEMORY_WRITE_POLICY", "1").lower() in {"1", "true", "yes", "on"}
        self.enable_profile_policy = os.getenv("ENABLE_PROFILE_WRITE_POLICY", "1").lower() in {"1", "true", "yes", "on"}
    ...
    def should_write_memory(self, response_text: str, confidence: Optional[float] = None) -> bool:
        ...
        if len(response_text.strip()) < self.min_response_length:
            logger.debug(
                ...
                extra={"meta": {"length": len(response_text), "min_required": self.min_response_length}}
            )
            return False
    ...
    def should_write_profile(self, response_text: str, profile_key: str, confidence: Optional[float] = None) -> bool:
        ...
        if len(response_text.strip()) < self.min_profile_response_length:
            logger.debug(
                ...
                extra={"meta": {"length": len(response_text), "min_required": self.min_profile_response_length}}
            )
            return False
        ...
        # Check confidence score if provided
        if confidence is not None and confidence < 0.8:  # Higher threshold for profile writes
            logger.debug(
        ...
        logger.debug("Profile write allowed", extra={"meta": {"profile_key": profile_key}})
        return True

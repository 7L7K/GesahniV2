...
def test_race_condition_fix_basic_concurrent(monkeypatch):
    ...
    with client:
        ...
        assert (
            HTTPStatus.SERVICE_UNAVAILABLE not in results
        ), "Should not get 503 errors"
...
def test_race_condition_fix_multiple_concurrent(monkeypatch):
    ...
    with client:
        ...
        assert (
            results.count(HTTPStatus.UNAUTHORIZED) == 4
        ), "Four requests should fail with 401"
        assert (
            HTTPStatus.SERVICE_UNAVAILABLE not in results
        ), "Should not get 503 errors"
...
def test_race_condition_fix_cookie_mode(monkeypatch):
    ...
    with client:
        ...
        assert results.count(HTTPStatus.OK) == 1, "Exactly one request should succeed"
        assert (
            results.count(HTTPStatus.UNAUTHORIZED) == 2
        ), "Two requests should fail with 401"
        assert (
            HTTPStatus.SERVICE_UNAVAILABLE not in results
        ), "Should not get 503 errors"
...
def test_race_condition_fix_retry_mechanism(monkeypatch):
    ...
    with client:
        ...
        assert results.count(HTTPStatus.OK) == 1, "Exactly one request should succeed"
        assert (
            results.count(HTTPStatus.UNAUTHORIZED) == 3
        ), "Three requests should fail with 401"
        assert (
            HTTPStatus.SERVICE_UNAVAILABLE not in results
        ), "Should not get 503 errors"
    ...
    with client:
        ...
        for status_code, body in [result1, result2]:
            if status_code == HTTPStatus.OK:
                ...
                assert (
                    body.get("detail") == "refresh_reused"
                ), "Failed response should have correct error detail"
...
def test_race_condition_fix_different_sessions(monkeypatch):
    ...
    with client1, client2:
        ...
        def call_refresh_user1():
            ...
            cc = TestClient(app)
            return cc.post(
                "/v1/auth/refresh", json={"refresh_token": refresh_token1}
            ).status_code
        ...
        def call_refresh_user2():
            ...
            cc = TestClient(app)
            return cc.post(
                "/v1/auth/refresh", json={"refresh_token": refresh_token2}
            ).status_code
        ...
        assert (
            results.count(HTTPStatus.OK) == 2
        ), "Both users should have one successful refresh"
        assert (
            results.count(HTTPStatus.UNAUTHORIZED) == 2
        ), "Both users should have one failed refresh"
        assert (
            HTTPStatus.SERVICE_UNAVAILABLE not in results
        ), "Should not get 503 errors"

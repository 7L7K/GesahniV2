...
async def temp_db():
    ...
    # Ensure tables exist (using PostgreSQL)
    await ensure_tables()

    # Create DAO with default PostgreSQL path (configured by conftest.py)
    dao = TokenDAO()
...
async def create_test_identity():
    """Factory to create test identity with a proper user."""

    async def _create_identity(user_id_str: str):
        # Create a proper user with UUID
...
async def test_spotify_integrations_status_with_token(temp_db, create_test_identity):
    ...
    user_id, identity_id = await create_test_identity("test_user_with_token")
    ...
    token = ThirdPartyToken(
        ...
        access_token="B" + "A" * 30,  # At least 20 chars
        refresh_token="A" + "B" * 30,  # At least 20 chars
...
async def test_spotify_integrations_status_expired_token(temp_db, create_test_identity):
    ...
    # Create test identity and token
    user_id, identity_id = await create_test_identity("test_user_expired")
...
async def test_spotify_integrations_status_recently_refreshed(
    ...
    # Create test identity and token
    user_id, identity_id = await create_test_identity("test_user_refreshed")

...
def test_scope_guard_denies_without_scope():
    c = TestClient(_app())
    token = jwt.encode({"user_id": "u", "scope": "music:control"}, os.getenv("JWT_SECRET", "secret"), algorithm="HS256")
    r = c.get("/admin", headers={"Authorization": f"Bearer {token}"})
    assert r.status_code == 403

    token2 = jwt.encode({"user_id": "u", "scope": "music:control admin:write"}, os.getenv("JWT_SECRET", "secret"), algorithm="HS256")

...
def app():
    ...
    async def test_verify_token_route(
        request: Request, _: None = Depends(verify_token)
    ):
        return JSONResponse({"status": "ok"})

    @app.options("/test-verify-token-strict")
    async def test_verify_token_strict_route(
        request: Request, _: None = Depends(verify_token_strict)
    ):
        return JSONResponse({"status": "ok"})

    @app.options("/test-rate-limit-with")
    async def test_rate_limit_with_route(
        request: Request, _: None = Depends(rate_limit_with(burst_limit=3))
    ):
        return JSONResponse({"status": "ok"})

    @app.options("/test-scope-rate-limit")
    async def test_scope_rate_limit_route(
        request: Request, _: None = Depends(scope_rate_limit("admin", burst_limit=3))
    ):
        return JSONResponse({"status": "ok"})

    @app.options("/test-rate-limit-problem")
    async def test_rate_limit_problem_route(
        request: Request, _: None = Depends(rate_limit_problem)
    ):
        return JSONResponse({"status": "ok"})

    @app.options("/test-require-nonce")
    async def test_require_nonce_route(
        request: Request, _: None = Depends(require_nonce)
    ):
        return JSONResponse({"status": "ok"})

    @app.options("/test-require-roles")
    async def test_require_roles_route(
        request: Request, _: None = Depends(require_roles(["admin"]))
    ):
        return JSONResponse({"status": "ok"})
    ...
    async def test_require_scope_route(
        request: Request, _: None = Depends(require_scope("admin"))
    ):
        return JSONResponse({"status": "ok"})

    @app.options("/test-require-any-scope")
    async def test_require_any_scope_route(
        request: Request, _: None = Depends(require_any_scope(["admin", "user"]))
    ):
        return JSONResponse({"status": "ok"})

    @app.options("/test-optional-require-scope")
    async def test_optional_require_scope_route(
        request: Request, _: None = Depends(optional_require_scope("admin"))
    ):
        return JSONResponse({"status": "ok"})

    @app.options("/test-optional-require-any-scope")
    async def test_optional_require_any_scope_route(
        request: Request,
        _: None = Depends(optional_require_any_scope(["admin", "user"])),
    ):
        return JSONResponse({"status": "ok"})

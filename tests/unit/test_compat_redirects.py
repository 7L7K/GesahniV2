...
class TestCompatRedirects:
    ...
    @pytest.mark.parametrize(
        "legacy_method,legacy_path,canonical_path",
        [
    ...
    def test_legacy_redirects(
        ...
        ), f"Expected 308 for {legacy_method} {legacy_path}, got {response.status_code}"

        # Assert Location header points to canonical path
        assert (
            response.headers.get("location") == canonical_path
        ...
        assert (
            len(response.content) == 0 or response.content == b""
        ), f"Expected empty body for redirect {legacy_method} {legacy_path}, got {len(response.content)} bytes"

    @pytest.mark.parametrize(
        "legacy_method,legacy_path,canonical_path,expected_status",
        [
    ...
    def test_redirects_in_openapi_schema(self, client: TestClient):
        ...
        assert canonical_paths_exist, "Should have some /v1/ paths in the schema"
    ...
    def test_legacy_auth_paths_hidden_from_schema(
        self, client: TestClient, legacy_path: str, canonical_path: str
    ):
    ...
    def test_no_compat_paths_leak_into_schema(self, client: TestClient):
        ...
        for auth_path in [
            ...
            if auth_path in paths:
                ...
                assert (
                    "post" in operations
                ), f"Auth path {auth_path} missing POST operation"

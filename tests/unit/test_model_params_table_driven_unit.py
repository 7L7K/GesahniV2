...
class TestBaseDefaults:
    ...
    def test_base_defaults_env_variations(
        self, monkeypatch, env_vars: dict[str, str], expected: dict[str, Any]
    ):
    ...
    @pytest.mark.parametrize(
        "env_var,invalid_value,expected_default",
        [
            ...
            (
                ...
                "xyz",
                None,
            ),  # max_completion_tokens defaults to None
        ],
    )
    def test_base_defaults_invalid_values(
        self, monkeypatch, env_var: str, invalid_value: str, expected_default: Any
    ):
...
class TestMergeParams:
    ...
    def test_merge_params_variations(
        self,
        monkeypatch,
    ...
    def test_merge_params_stop_normalization(
        self, monkeypatch, stop_input: Any, expected_output: list[str] | None
    ):
...
class TestForOpenAI:
    ...
    @pytest.mark.parametrize(
        "overrides,expected_openai_args",
        [
            # Basic mapping
            ...
            (
                {"max_tokens": 100},
                {"temperature": 0.1, "top_p": 0.9, "max_completion_tokens": 100},
            ),
            (
    ...
    def test_for_openai_mapping(
        self,
        monkeypatch,
        overrides: dict[str, Any] | None,
        expected_openai_args: dict[str, Any],
    ):
...
class TestForOllama:
    ...
    @pytest.mark.parametrize(
        "overrides,expected_ollama_args",
        [
            # Basic mapping
            (None, {"temperature": 0.1, "top_p": 0.9}),
            ({}, {"temperature": 0.1, "top_p": 0.9}),
            ...
            (
                {"max_tokens": 100},
                {"temperature": 0.1, "top_p": 0.9, "num_predict": 100},
            ),
    ...
    def test_for_ollama_mapping(
        self,
        monkeypatch,
        overrides: dict[str, Any] | None,
        expected_ollama_args: dict[str, Any],
    ):

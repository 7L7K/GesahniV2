...
class TestMusicRouting:
    ...
    def test_legacy_spotify_integrations_redirects(
        ...
        ), f"Expected 308 for GET {legacy_path}, got {response.status_code}"
    ...
    def test_openapi_schema_includes_canonical_music_routes(self, client: TestClient):
        ...
        for path in canonical_music_paths:
            ...
            assert (
                path_found
            ), f"Canonical music path {path} should appear in OpenAPI schema"
    ...
    def test_openapi_schema_includes_canonical_spotify_routes(self, client: TestClient):
        ...
        for path in canonical_spotify_paths:
            assert (
                path in paths
            ), f"Canonical Spotify path {path} should appear in OpenAPI schema"

    def test_openapi_schema_excludes_legacy_integrations_paths(
    ...
    def test_no_duplicate_router_entries(self, client: TestClient):
        ...
        assert (
            len(duplicates) == 0
        ), f"Found duplicate paths in OpenAPI schema: {duplicates}"
        ...
        for path in spotify_paths:
            ...
            for method, operation in operations.items():
                if method.lower() in ["get", "post", "put", "delete"]:
                    ...
                    assert isinstance(
                        tags, list
                    ), f"Path {path} {method} should have tags"
    ...
    def test_redirect_methods_preserved(

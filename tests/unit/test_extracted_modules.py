...
class TestRouterPolicy:
    ...
    def test_pick_model_with_policy_override(
        self, mock_pick_model, mock_count_tokens, mock_detect_intent
    ):
        ...
        decision = pick_model_with_policy(
            prompt="test prompt", model_override="gpt-4o", allow_fallback=False
        )
    ...
    @patch("app.router_policy.count_tokens")
    @patch("app.router_policy.pick_model")
    def test_pick_model_with_policy_automatic(
        self, mock_pick_model, mock_count_tokens, mock_detect_intent
    ):
        ...
        decision = pick_model_with_policy(prompt="test prompt", allow_fallback=True)
...
class TestAdapters:
    ...
    async def test_call_llm_openai(self, mock_call_openai):
        ...
        request = LLMRequest(prompt="test prompt", model="gpt-4o")
    ...
    async def test_call_llm_ollama(self, mock_call_ollama):
        ...
        request = LLMRequest(prompt="test prompt", model="llama3:latest")
...
class TestPostCall:
    ...
    @pytest.mark.asyncio
    async def test_process_postcall(
        self, mock_cache, mock_claims, mock_memory, mock_analytics, mock_history
    ):
...
class TestHealth:
    ...
    def test_health_check_result_creation(self):
        ...
        result = HealthCheckResult(
            healthy=True, status="healthy", latency_ms=100.0, timestamp=1234567890.0
        )
    ...
    async def test_health_check_cache(self):
        ...
        result = HealthCheckResult(healthy=True, status="healthy", latency_ms=100.0)
    ...
    async def test_check_system_health(self, mock_ollama, mock_openai):
        ...
        mock_openai.return_value = HealthCheckResult(
            healthy=True, status="healthy", latency_ms=100.0
        )
        mock_ollama.return_value = HealthCheckResult(
            healthy=True, status="healthy", latency_ms=200.0
        )

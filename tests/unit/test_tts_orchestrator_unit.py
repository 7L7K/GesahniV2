...
def test_tts_cache_hit(monkeypatch):
    ...
    audio1 = tts.__dict__["asyncio"].get_event_loop().run_until_complete(
        tts.synthesize(text="hello", mode="utility")
    )
    assert isinstance(audio1, (bytes, bytearray))
    # Second call should return cached bytes quickly
    audio2 = tts.__dict__["asyncio"].get_event_loop().run_until_complete(
        tts.synthesize(text="hello", mode="utility")
    )
...
def test_tts_daily_cap_autodegrade(monkeypatch):
    ...
    audio = tts.__dict__["asyncio"].get_event_loop().run_until_complete(
        tts.synthesize(text="status ok", mode="utility")
    )

...
class TestLoadEnvPrecedence:
    ...
    @pytest.mark.parametrize("env_files,existing_env,expected_values", [
    ...
    def test_load_env_precedence(self, monkeypatch, tmp_path: Path, env_files: Dict[str, str], 
        ...
        for key, expected_value in expected_values.items():
            assert os.getenv(key) == expected_value, f"Expected {key}={expected_value}, got {os.getenv(key)}"


class TestLoadEnvForceReload:
    """Table-driven tests for force reload behavior."""
    
    @pytest.mark.parametrize("force_value,should_reload", [
        (True, True),
        (False, False),
        (1, True),
        (0, False),
    ...
    def test_load_env_force_values(self, monkeypatch, tmp_path: Path, force_value, should_reload: bool):
...
class TestLoadEnvTestMode:
    ...
    @pytest.mark.parametrize("test_env_vars,should_bypass_cache", [
...
class TestLoadEnvFileHandling:
    ...
    @pytest.mark.parametrize("file_scenarios,expected_behavior", [
        # Missing files
        ({}, "no_files"),
...
class TestLoadEnvCaching:
    ...
    @pytest.mark.parametrize("file_changes,should_reload", [
        # No changes
        ({}, False),
...
class TestLoadEnvErrorHandling:
    ...
    def test_load_env_error_handling(self, monkeypatch, tmp_path: Path, error_scenario: str, expected_behavior: str):
        ...
        with patch("app.env_utils.dotenv_values", side_effect=mock_dotenv_values):
            ...
            with pytest.raises((PermissionError, UnicodeDecodeError, ValueError, OSError)):
                env_utils.load_env()
...
class TestLoadEnvLogging:
    ...
    @pytest.mark.parametrize("scenario,expected_log_calls", [
        # First load
        ("first_load", 1),
    ...
    def test_load_env_logging(self, monkeypatch, tmp_path: Path, scenario: str, expected_log_calls: int):
...
class TestLoadEnvBackCompat:
    ...
    @pytest.mark.parametrize("legacy_cache_state,expected_behavior", [
    ...
    def test_load_env_back_compat(self, monkeypatch, tmp_path: Path, legacy_cache_state, expected_behavior: str):
...
class TestLoadEnvEnvironmentSpecific:
    ...
    @pytest.mark.parametrize("env_files,expected_vars", [
        # Development environment
        ({"env.dev": "DEV_VAR=dev_value\nDEV_ONLY=dev_only"}, {"DEV_VAR": "dev_value", "DEV_ONLY": "dev_only"}),
        
        # Staging environment
        ({"env.staging": "STAGING_VAR=staging_value"}, {"STAGING_VAR": "staging_value"}),
        
        # Production environment
        ({"env.prod": "PROD_VAR=prod_value"}, {"PROD_VAR": "prod_value"}),
        
        # Localhost environment
        ({"env.localhost": "LOCALHOST_VAR=localhost_value"}, {"LOCALHOST_VAR": "localhost_value"}),

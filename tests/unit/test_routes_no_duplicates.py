...
def test_no_duplicate_core_routes():
    ...
    for methods, path in [ (('GET',), '/v1/whoami'), (('GET',), '/v1/sessions'), (('POST',), '/v1/auth/logout'), (('POST',), '/v1/auth/refresh') ]:
        key = (methods, path)
        matches = [rt for k, v in paths.items() if k[1] == path and methods[0] in k[0] for rt in v]

...
class TestCookieJWTCompliance:
    ...
    def run_grep_command(
        self,
        pattern: str,
        include_pattern: str = "app/**/*.py",
        exclude_pattern: str = "tests/**",
    ) -> list[str]:
    ...
    def test_no_unauthorized_set_cookie_calls(self):
        ...
        assert (
            len(violations) == 0
        ), "Found unauthorized set_cookie() calls:\n" + "\n".join(violations)
    ...
    def test_no_unauthorized_set_cookie_headers(self):
        ...
        for line in set_cookie_header_lines:
            if line.strip():
                ...
                if file_path not in self.ALLOWED_COOKIE_FILES:
                    violations.append(
                        f"Direct Set-Cookie header manipulation in {line}"
                    )

        assert (
            len(violations) == 0
        ), "Found unauthorized Set-Cookie header manipulation:\n" + "\n".join(
            violations
        )
    ...
    def test_no_unauthorized_jwt_encode_calls(self):
        ...
        assert (
            len(violations) == 0
        ), "Found unauthorized jwt.encode() calls:\n" + "\n".join(violations)
    ...
    def test_app_token_minting_goes_through_tokens_py(self):
        ...
        assert (
            len(violations) == 0
        ), "Found unauthorized app token minting:\n" + "\n".join(violations)
    ...
    def test_idp_token_signing_only_in_allowed_locations(self):
        ...
        assert (
            len(violations) == 0
        ), "Found unauthorized IdP token signing:\n" + "\n".join(violations)
    ...
    def test_auth_py_uses_centralized_functions(self):
        ...
        with open(auth_file) as f:
            ...
            assert (
                "set_auth_cookies" in content
            ), "auth.py should use set_auth_cookies from cookies.py"

            # Check for centralized token usage
            assert (
                "make_access" in content or "make_refresh" in content
            ), "auth.py should use make_access/make_refresh from tokens.py"
    ...
    def test_cookie_config_centralized_configuration_exists(self):
        ...
        assert os.path.exists(
            config_file
        ), f"Cookie config file {config_file} not found"
        ...
        with open(config_file) as f:
            ...
            for func in required_functions:
                assert (
                    func in content
                ), f"cookie_config.py should provide {func} function"
    ...
    def test_no_direct_cookie_manipulation_in_application_code(self):
        ...
        assert (
            len(violations) == 0
        ), "Found unauthorized cookie manipulation:\n" + "\n".join(violations)
        ...
        for line in es256_lines:
            if line.strip():
                ...
                if file_path not in self.IDP_TOKEN_SIGNING_FILES:
                    if not file_path.startswith("app/integrations/google/"):
                        pytest.fail(
                            f"IdP token signing outside allowed locations: {line}"
                        )

            ...
            with patch('app.api.google_oauth._verify_signed_state', return_value=True):
                with patch('app.integrations.google.oauth.exchange_code') as mock_exchange:
                    ...
                    with patch('jwt.decode') as mock_decode:
                        mock_decode.return_value = {"email": "test@example.com", "sub": "123"}
                        ...
                        with patch('app.api.google_oauth.set_auth_cookies'):
                            ...
                            assert success_log_call is not None, "oauth.callback.success log not found"
        ...
        with patch('app.api.google_oauth.logger') as mock_logger:
            with patch('app.api.google_oauth._verify_signed_state', return_value=True):
                with patch('app.integrations.google.oauth.exchange_code') as mock_exchange:
                    ...
                    assert fail_log_call is not None, "oauth.callback.fail log not found"
                ...
                with patch('app.integrations.google.oauth.exchange_code') as mock_exchange:
                    ...
                    with patch('jwt.decode') as mock_decode:
                        ...
                        with patch('app.api.google_oauth.set_auth_cookies'):
                            ...
                            for call in call_args:
                                if 'http_out' in call[1].get('extra', {}).get('meta', {}):
                                    http_out_log_call = call
                                    break
                            
                            assert http_out_log_call is not None, "http_out log not found"

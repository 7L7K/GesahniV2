...
class TestHealthCheckImprovements:
    ...
    async def test_exponential_backoff_before_success(self, mock_scheduler, mock_json_request):
        """Test exponential backoff behavior before first success."""
        # Mock environment
        with patch.dict('os.environ', {'OLLAMA_MODEL': 'test-model', 'OLLAMA_URL': 'http://test'}):
    ...
    async def test_success_resets_backoff(self, mock_scheduler, mock_json_request):
        ...
        with patch.dict('os.environ', {'OLLAMA_MODEL': 'test-model', 'OLLAMA_URL': 'http://test'}):
            ...
            assert llama_health_check_state["next_check_delay"] == 5.0  # Reset to initial
    ...
    async def test_throttling_after_success(self, mock_scheduler, mock_json_request):
        ...
        with patch.dict('os.environ', {'OLLAMA_MODEL': 'test-model', 'OLLAMA_URL': 'http://test'}):
            ...
            llama_health_check_state["last_success_ts"] = time.monotonic() - 30.0  # 30 seconds ago
            await _check_and_set_flag()
    ...
    @patch('app.llama_integration.json_request')
    @patch('app.llama_integration.scheduler')
    async def test_startup_check_schedules_health_check(self, mock_scheduler, mock_json_request):
    ...
    @patch('app.llama_integration.json_request')
    @patch('app.llama_integration.scheduler')
    async def test_skip_health_check_due_to_throttling(self, mock_scheduler, mock_json_request):
    ...
    @patch('app.llama_integration.json_request')
    @patch('app.llama_integration.scheduler')
    async def test_skip_health_check_due_to_backoff(self, mock_scheduler, mock_json_request):
        """Test that health checks are skipped during exponential backoff."""
        # Mock environment
        with patch.dict('os.environ', {'OLLAMA_MODEL': 'test-model', 'OLLAMA_URL': 'http://test'}):
            # Set up as if we recently failed and are in backoff
            llama_health_check_state["last_check_ts"] = time.monotonic() - 2.0  # 2 seconds ago
            llama_health_check_state["next_check_delay"] = 10.0
    ...
    @patch('app.llama_integration.json_request')
    @patch('app.llama_integration.scheduler')
    async def test_health_check_proceeds_when_not_throttled(self, mock_scheduler, mock_json_request):
        """Test that health checks proceed when not throttled."""
        # Mock environment
        with patch.dict('os.environ', {'OLLAMA_MODEL': 'test-model', 'OLLAMA_URL': 'http://test'}):
            # Set up as if we succeeded long ago
            llama_health_check_state["has_ever_succeeded"] = True
            llama_health_check_state["last_success_ts"] = time.monotonic() - 120.0  # 2 minutes ago

...
def test_login_sets_cookies_once(monkeypatch):
    ...
    assert (
        len(access_cookies) == 1
    ), f"Expected 1 access_token cookie, got {len(access_cookies)}"
    assert (
        len(refresh_cookies) == 1
    ), f"Expected 1 refresh_token cookie, got {len(refresh_cookies)}"
    assert (
        len(session_cookies) == 1
    ), f"Expected 1 __session cookie, got {len(session_cookies)}"
...
def test_logout_clears_cookies_properly(monkeypatch):
    ...
    assert (
        len(access_cookies) == 1
    ), f"Expected 1 access_token clear cookie, got {len(access_cookies)}"
    assert (
        len(refresh_cookies) == 1
    ), f"Expected 1 refresh_token clear cookie, got {len(refresh_cookies)}"
    assert (
        len(session_cookies) == 1
    ), f"Expected 1 __session clear cookie, got {len(session_cookies)}"
    ...
    invalid_refresh_resp = client.post(

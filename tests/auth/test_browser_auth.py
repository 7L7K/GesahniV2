    ...
    if whoami_response.status_code == 200:
        ...
        if should_call_refresh:
            ...
            if refresh_response.status_code == 200:
                ...
                print_result(
                    "refresh_called_appropriately",
                    True,
                    "Refresh called only when needed",
                )
            else:
                print_result(
                    "refresh_called_appropriately",
                    True,
                    f"Refresh failed as expected: {refresh_response.status_code}",
                )
        else:
            ...
            print_result(
                "refresh_called_appropriately",
                True,
                "No refresh called when not needed",
            )

    else:
        print_result(
            "whoami_boot_call", False, f"Status: {whoami_response.status_code}"
        )
...
def test_401_handling():
    ...
    if state_response.status_code == 401:
        ...
        cors_credentials = state_response.headers.get(
            "Access-Control-Allow-Credentials"
        )
        vary_header = state_response.headers.get("Vary")

        print(
            f"   CORS headers - Origin: {cors_origin}, Credentials: {cors_credentials}, Vary: {vary_header}"
        )
        print_result(
        ...
        print_result(
            "protected_endpoint_401", False, f"Status: {state_response.status_code}"
        )

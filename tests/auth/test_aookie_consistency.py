...
# Some environments place helpers under tests/helpers/test_helpers.py
# Import via package path fallback when the direct import is unavailable.
try:
    ...
    from tests.helpers.test_helpers import (  # type: ignore
        assert_cookies_cleared,
        assert_cookies_present,
        assert_session_opaque,
    )
...
def test_session_cookie_is_opaque_and_consistent(client, mock_user_store):
    ...
    assert (
        "__session" not in refresh_cookies
    ), "__session should not be set during refresh"

    # Verify that the original session cookie is still valid and opaque
    assert (
        "access_token" in login_cookies
    ), "access_token cookie not found in login response"

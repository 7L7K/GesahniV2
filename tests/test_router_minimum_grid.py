...
def reset_router_state(monkeypatch):
    ...
    # Reload allow-lists
    router.ALLOWED_GPT_MODELS, router.ALLOWED_LLAMA_MODELS = router._get_allowed_models()
...
class TestGoldenTraceEmitsOnceWithFields:
    ...
    async def test_golden_trace_emits_once_with_fields(self, monkeypatch):
        ...
        with patch('app.gpt_client.ask_gpt') as mock_gpt:
            ...
            trace_kwargs = trace_calls[0][1]
            required_fields = [
                "request_id", "user_id", "path", "shape", "intent", 
                "tokens_est", "routing_decision", "dry_run", 
                "cb_user_open", "cb_global_open", "cache_hit"
            ]

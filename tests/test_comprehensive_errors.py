...
class TestComprehensiveErrors:
    ...
    def test_concurrent_token_creation(self):
        ...
        with ThreadPoolExecutor(max_workers=10) as executor:
            futures = [
                executor.submit(create_token, f"user_{i}")
                for i in range(100)
            ]
    ...
    def test_concurrent_requests(self):
        ...
        request_ids = [resp.headers.get("X-Request-ID") for resp in responses if resp.headers.get("X-Request-ID")]
    ...
    def test_malformed_requests(self):
        ...
        response = self.client.post("/ask", data="invalid json", headers={"Content-Type": "application/json"})
        ...
        response = self.client.post("/ask", data="test", headers={"Content-Type": "text/plain"})
    ...
    def test_performance_degradation(self):
        ...
        assert total_time < 10.0, f"Performance degradation detected: {total_time}s for 100 requests"
    ...
    def test_logging_under_load(self):
        ...
        assert logging_time < 1.0, f"Logging performance issue: {logging_time}s for 1000 messages"
...
class TestIntegrationFailures:
    ...
    def test_data_corruption_scenarios(self):
        ...
        client = TestClient(app)
        response = client.get("/health", headers={"Authorization": f"Bearer {corrupted_token}"})

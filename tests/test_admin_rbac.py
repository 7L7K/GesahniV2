...
def test_admin_forbidden():
    client = _spin()
    r = client.get("/v1/admin/ping", headers={"Authorization": f"Bearer {_tok(['user'])}"})
    assert r.status_code == 403

def test_admin_ok():
    client = _spin()
    r = client.get("/v1/admin/ping", headers={"Authorization": f"Bearer {_tok(['admin'])}"})
...
def test_admin_rbac_info_forbidden():
    client = _spin()
    r = client.get("/v1/admin/rbac/info", headers={"Authorization": f"Bearer {_tok(['user'])}"})
    assert r.status_code == 403

def test_admin_rbac_info_ok():
    client = _spin()
    r = client.get("/v1/admin/rbac/info", headers={"Authorization": f"Bearer {_tok(['admin:read'])}"})
...
def test_admin_users_me_ok():
    client = _spin()
    r = client.get("/v1/admin/users/me", headers={"Authorization": f"Bearer {_tok(['user:profile'])}"})
    assert r.status_code == 200
    data = r.json()
...
def test_admin_system_status_forbidden():
    client = _spin()
    r = client.get("/v1/admin/system/status", headers={"Authorization": f"Bearer {_tok(['user'])}"})
    assert r.status_code == 403

def test_admin_system_status_ok():
    client = _spin()
    r = client.get("/v1/admin/system/status", headers={"Authorization": f"Bearer {_tok(['admin:read'])}"})
    assert r.status_code == 200
    data = r.json()

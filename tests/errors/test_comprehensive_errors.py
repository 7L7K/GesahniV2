...
class TestComprehensiveErrors:
    ...
    def test_llama_integration_failures(self, event_loop):
        ...
        # Test when LLAMA_ENABLED=0 - should raise specific error
        with patch.dict(os.environ, {"LLAMA_ENABLED": "0"}):
            with pytest.raises(
                RuntimeError, match="LLaMA integration disabled via LLAMA_ENABLED=0"
            ):
        ...
        if "LLAMA_ENABLED" in os.environ:
            del os.environ["LLAMA_ENABLED"]

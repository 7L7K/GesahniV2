    ...
    def record_request(self, method, url, status_code=None):
        ...
        if status_code == 401:
            self.four_oh_ones.append(
                {"method": method, "url": url, "timestamp": time.time()}
            )
        ...
        if name in self.connections:
            ...
            if self.reconnect_attempts > 1:  # Max one reconnect attempt
                ...
                self.disconnect_events.append(
                    {"name": name, "reason": reason, "timestamp": time.time()}
                )
...
class TestAuthenticationBehavior:
    ...
    def test_boot_logged_out_to_logged_in(
        self, client, mock_network_panel, mock_auth_orchestrator, auth_tokens
    ):
        ...
        finisher_response = client.post(
            "/v1/auth/refresh", headers={"X-Auth-Intent": "refresh"}
        )
        assert finisher_response.status_code in [
            200,
            401,
        ...
        assert (
            mock_auth_orchestrator.whoami_calls == 1
        ), "Should have exactly one whoami call"
    ...
    def test_refresh_while_logged_in(self, client, mock_auth_orchestrator, auth_tokens):
        ...
        assert (
            mock_auth_orchestrator.whoami_calls == 1
        ), "Should have exactly one whoami on mount"

        # Verify no additional auth state flips
        auth_changes = mock_auth_orchestrator.auth_state_changes
        assert (
            len(auth_changes) == 0
        ), "Should have no auth state changes during refresh"

        # Verify no component makes its own whoami calls
        # (This would be enforced by the AuthOrchestrator singleton pattern)
        assert (
            mock_auth_orchestrator.whoami_calls == 1
        ), "Should still have only one whoami call"

    def test_logout_behavior(
        self, client, mock_auth_orchestrator, mock_network_panel, auth_tokens
    ):
        ...
        with patch("app.deps.user.get_current_user_id", return_value="anon"):
            ...
            assert (
                privileged_response.status_code == 401
            ), "Privileged calls should fail after logout"
    ...
    def test_websocket_behavior(self, mock_ws_hub, mock_auth_orchestrator):
        ...
        assert (
            mock_ws_hub.connection_attempts == 0
        ), "Should not attempt connection when not authenticated"

        # 2. Test connection when authenticated
        mock_auth_orchestrator.setAuthenticated(True)
        mock_ws_hub.start({"music": True, "care": True})
        assert (
            mock_ws_hub.connection_attempts == 1
        ), "Should attempt connection when authenticated"
        ...
        assert (
            mock_ws_hub.reconnect_attempts == 2
        ), "Should have attempted two reconnects"
    ...
    def test_health_check_behavior(self, mock_health_checker, mock_auth_orchestrator):
        ...
        assert (
            final_auth_changes == initial_auth_changes
        ), "Health checks should not mutate auth state"

        # Verify health calls increased
        assert (
            mock_health_checker.health_calls == 6
        ), "Should have made 6 health calls total"
    ...
    def test_csp_service_worker_sanity(self, client):
        ...
        assert (
            "no-store" in cache_headers.lower() or "no-cache" in cache_headers.lower()

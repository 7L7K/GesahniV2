...
async def test_brand_account_same_email_different_sub(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(google_services, "get_token", lambda user, prov: dao.get_token(user, prov))
    monkeypatch.setattr(google_services, "get_all_user_tokens", lambda user: dao.get_all_user_tokens(user))
    monkeypatch.setattr(google_services, "get_current_user_id", lambda request=None: "u99")

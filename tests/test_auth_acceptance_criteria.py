...
class AuthAcceptanceTester:
    ...
    def test_zero_401s_during_boot(self) -> bool:
        ...
        try:
            ...
            if response.status_code == 401:
                ...
                self.log_test(
                    "Zero 401s during boot", True, "whoami correctly returns 200"
                )
            else:
                self.log_test(
                    "Zero 401s during boot",
                    False,
                    f"whoami returned unexpected status {response.status_code}",
                )
                return False
        except Exception as e:
            self.log_test(
                "Zero 401s during boot", False, f"Exception testing whoami: {e}"
            )
            return False
        ...
        try:
            ...
            self.log_test(
                "Protected endpoints require auth",
                False,
                f"Exception testing protected endpoint: {e}",
            )
            return False
    ...
    def test_finisher_and_whoami_sequence(self) -> bool:
        ...
        logger.info(
            "Testing: Exactly one finisher call per login and one whoami immediately after"
        )
        ...
        try:
            ...
            if response.status_code in [204, 302]:  # Expected responses
                self.log_test(
                    "Finisher endpoint exists",
                    True,
                    f"Auth finish endpoint returns {response.status_code}",
                )
            else:
                self.log_test(
                    "Finisher endpoint exists",
                    False,
                    f"Auth finish endpoint returned {response.status_code}",
                )
                return False
        except Exception as e:
            self.log_test(
                "Finisher endpoint exists", False, f"Exception testing auth finish: {e}"
            )
            return False
    ...
    def test_whoami_no_oscillation(self) -> bool:
        ...
        for i in range(5):
            try:
                ...
                self.log_test(
                    "whoamiOk no oscillation",
                    False,
                    f"Exception on whoami call {i}: {e}",
                )
                return False
        ...
        if len(set(responses)) == 1:
            self.log_test(
                "whoamiOk no oscillation",
                True,
                f"Authentication status consistent: {responses[0]}",
            )
        else:
            self.log_test(
                "whoamiOk no oscillation",
                False,
                f"Authentication status oscillated: {responses}",
            )
            return False
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if not missing_fields:
                    self.log_test(
                        "Only orchestrator calls whoami",
                        True,
                        "whoami endpoint returns correct structure",
                    )
                else:
                    self.log_test(
                        "Only orchestrator calls whoami",
                        False,
                        f"whoami missing fields: {missing_fields}",
                    )
                    return False
            else:
                self.log_test(
                    "Only orchestrator calls whoami",
                    False,
                    f"whoami returned {response.status_code}",
                )
                return False
        except Exception as e:
            self.log_test(
    ...
    def test_privileged_api_calls_gated(self) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Music state endpoint response: {response.status_code} - {response.text[:100]}"
            )
            if response.status_code == 401:
                self.log_test(
                    "Privileged API calls gated",
                    True,
                    "Music state endpoint correctly requires authentication",
                )
            else:
                self.log_test(
                    "Privileged API calls gated",
                    False,
                    f"Music state endpoint returned {response.status_code} instead of 401",
                )
                return False
        except Exception as e:
            self.log_test(
                "Privileged API calls gated",
                False,
                f"Exception testing music state: {e}",
            )
            return False
        ...
        try:
            response = fresh_session.post(
                f"{self.base_url}/v1/music", json={"command": "play"}
            )
            logger.info(
                f"Music control endpoint response: {response.status_code} - {response.text[:100]}"
            )
            if response.status_code == 401:
                self.log_test(
                    "Music control gated",
                    True,
                    "Music control endpoint correctly requires authentication",
                )
            else:
                self.log_test(
                    "Music control gated",
                    False,
                    f"Music control endpoint returned {response.status_code} instead of 401",
                )
                return False
        except Exception as e:
            self.log_test(
                "Music control gated", False, f"Exception testing music control: {e}"
            )
            return False
    ...
    def test_websocket_no_whoami_on_errors(self) -> bool:
        ...
        try:
            ...
            if response.status_code in [
                ...
                401,
                404,
            ]:  # Expected responses for WS endpoints
                self.log_test(
                    "WebSocket no whoami on errors",
                    True,
                    f"WebSocket endpoint correctly handles HTTP requests: {response.status_code}",
                )
            else:
                self.log_test(
                    "WebSocket no whoami on errors",
            ...
            self.log_test(
                "WebSocket no whoami on errors",
                False,
                f"Exception testing WebSocket: {e}",
            )
            return False
    ...
    def run_all_tests(self) -> dict[str, Any]:
        ...
        tests = [
            ...
            (
                "Only orchestrator calls whoami",
                self.test_only_orchestrator_calls_whoami,
            ),
    ...
    def print_summary(self):
        ...
        if passed == total:
            ...
            logger.info(
                "⚠️  Some acceptance criteria failed. Please review the implementation."
            )

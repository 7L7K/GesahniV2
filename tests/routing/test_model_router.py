...
def test_compose_cache_id_stable():
    cid1 = compose_cache_id("gpt-4.1-nano", "Hello World", ["doc A", "doc B"])
    cid2 = compose_cache_id(
        "gpt-4.1-nano", "Hello  World ", ["doc B", "doc A"]
    )  # reordered
    assert cid1 == cid2
    assert cid1.startswith("v1|gpt-4.1-nano|")
...
def test_route_text_ops_branches():
    ...
    assert d_simple.model == "gpt-4.1-nano"

...
def _generate_unique_username():
    ...
    return "contract_test_user_" + "".join(
...
def _get_csrf_token(client: TestClient) -> str:
    ...
    ), f"CSRF endpoint failed: {csrf_resp.status_code}"
...
def _make_authenticated_request(
    client: TestClient, method: str, url: str, with_csrf: bool = False, **kwargs
) -> TestClient:
...
def test_contract_matrix(client: TestClient):
    ...
    assert_status(
        r.status_code, {401}, "GET /v1/auth/whoami", "unauthenticated", r.text
    )
    ...
    ), f"CSRF endpoint failed: {r.status_code}"
    ...
    assert_status(
        r.status_code, {200, 204}, "POST /v1/auth/login", "unauthenticated", r.text
    )
...
def test_csrf_write_guards(auth_client, monkeypatch):
    ...
    assert_status(
        r.status_code, {200}, "POST /v1/auth/refresh", "authed + CSRF", r.text
    )
    ...
    assert_status(
        r.status_code, {400, 403}, "POST /v1/auth/logout_all", "authed no CSRF", r.text
    )

    # Test logout_all with CSRF should work
    r = auth_client.post("/v1/auth/logout_all")  # auth_client automatically adds CSRF
    assert_status(
        r.status_code, {204}, "POST /v1/auth/logout_all", "authed + CSRF", r.text
    )
    ...
    assert_status(
        r.status_code, {401}, "POST /v1/auth/logout", "after logout_all", r.text
    )
...
def test_public_endpoints_work_unauthenticated(client: TestClient):
    ...
    r = _follow(
        ...
        "POST",
        "/v1/auth/register",
        json={"username": "public_test_user", "password": "secret"},
    )
    assert_status(
        r.status_code,
        {200, 201, 409},

...
def test_google_oauth_flow():
    ...
    if response.status_code == 200:
        ...
        checks = [
            ...
            ('redirect_params' in params and 'next=' in params.get('redirect_params', [''])[0], 'next parameter'),
        ]
    ...
    callback_response = requests.get(f"{BASE_URL}/v1/google/auth/callback?state=invalid&code=invalid")
    ...
    status_response = requests.get(f"{BASE_URL}/v1/google/status", 
                                 headers={"Authorization": "Bearer invalid"})
    if status_response.status_code == 401:
        print("   ✅ Status endpoint properly requires authentication")
    else:
        print(f"   ⚠️  Status endpoint returned {status_response.status_code} (expected 401)")
    ...
    if integrations_response.status_code == 200:
        ...
        print(f"   ❌ Failed to get integrations status: {integrations_response.status_code}")

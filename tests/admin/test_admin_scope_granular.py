...
def test_admin_read_requires_read_or_admin_scope(client):
    ...
    response = client.get(
        "/v1/admin/config", headers={"Authorization": f"Bearer {token_read}"}
    )
    ...
    response = client.get(
        "/v1/admin/config", headers={"Authorization": f"Bearer {token_admin}"}
    )
    assert response.status_code == 200
    ...
    response = client.get(
        "/v1/admin/config", headers={"Authorization": f"Bearer {token_insufficient}"}
    )
...
def test_admin_metrics_endpoint_scope(client):
    ...
    token_read = _tok(["admin:read"])
    response = client.get(
        "/v1/admin/metrics", headers={"Authorization": f"Bearer {token_read}"}
    )
    assert response.status_code == 200
    ...
    token_admin = _tok(["admin"])
    response = client.get(
        "/v1/admin/metrics", headers={"Authorization": f"Bearer {token_admin}"}
    )
    assert response.status_code == 200
    ...
    token_insufficient = _tok(["user:profile"])
    response = client.get(
        "/v1/admin/metrics", headers={"Authorization": f"Bearer {token_insufficient}"}
    )
    assert response.status_code == 403
...
def test_admin_system_status_endpoint_scope(client):
    ...
    token_read = _tok(["admin:read"])
    response = client.get(
        "/v1/admin/system/status", headers={"Authorization": f"Bearer {token_read}"}
    )
    assert response.status_code == 200
    ...
    token_admin = _tok(["admin"])
    response = client.get(
        "/v1/admin/system/status", headers={"Authorization": f"Bearer {token_admin}"}
    )
    assert response.status_code == 200
    ...
    response = client.get(
        ...
        headers={"Authorization": f"Bearer {token_insufficient}"},
    )
    assert response.status_code == 403
...
def test_admin_flags_endpoint_scope(client):
    ...
    token_read = _tok(["admin:read"])
    response = client.get(
        "/v1/admin/flags", headers={"Authorization": f"Bearer {token_read}"}
    )
    assert response.status_code == 200
    ...
    token_admin = _tok(["admin"])
    response = client.get(
        "/v1/admin/flags", headers={"Authorization": f"Bearer {token_admin}"}
    )
    assert response.status_code == 200
    ...
    token_insufficient = _tok(["user:profile"])
    response = client.get(
        "/v1/admin/flags", headers={"Authorization": f"Bearer {token_insufficient}"}
    )
    assert response.status_code == 403
...
def test_admin_errors_endpoint_scope(client):
    ...
    token_read = _tok(["admin:read"])
    response = client.get(
        "/v1/admin/errors", headers={"Authorization": f"Bearer {token_read}"}
    )
    assert response.status_code == 200
    ...
    token_admin = _tok(["admin"])
    response = client.get(
        "/v1/admin/errors", headers={"Authorization": f"Bearer {token_admin}"}
    )
    assert response.status_code == 200
    ...
    token_insufficient = _tok(["user:profile"])
    response = client.get(
        "/v1/admin/errors", headers={"Authorization": f"Bearer {token_insufficient}"}
    )
    assert response.status_code == 403
...
def test_user_profile_endpoint_scope(client):
    ...
    response = client.get(
        "/v1/admin/users/me", headers={"Authorization": f"Bearer {token_profile}"}
    )
    assert response.status_code == 200
    ...
    token_admin = _tok(["admin"])
    response = client.get(
        "/v1/admin/users/me", headers={"Authorization": f"Bearer {token_admin}"}
    )
    assert response.status_code == 200
    ...
    response = client.get(
        "/v1/admin/users/me", headers={"Authorization": f"Bearer {token_insufficient}"}
    )
    assert response.status_code == 403
...
def test_scope_combination_scenarios(client):
    ...
    response = client.get(
        "/v1/admin/config", headers={"Authorization": f"Bearer {token_multi}"}
    )
    assert response.status_code == 200
    ...
    token_empty = _tok([])
    response = client.get(
        "/v1/admin/config", headers={"Authorization": f"Bearer {token_empty}"}
    )
    assert response.status_code == 403
    ...
    response = client.get(
        "/v1/admin/config", headers={"Authorization": f"Bearer {token_invalid}"}
    )
    assert response.status_code == 403
...
def test_scope_inheritance_admin_full_access(client):
    ...
    for endpoint in endpoints:
        response = client.get(
            endpoint, headers={"Authorization": f"Bearer {token_admin}"}
        )
...
def test_scope_separation_read_vs_write(client):
    ...
    for endpoint in read_endpoints:
        response = client.get(
            endpoint, headers={"Authorization": f"Bearer {token_read}"}
        )
    ...
    for endpoint, method in write_endpoints:
        if method == "POST":
            ...
            response = client.put(
                endpoint, json={}, headers={"Authorization": f"Bearer {token_read}"}
            )
        assert (
            response.status_code == 403
        ), f"admin:read should not allow {method} {endpoint}"
...
def test_expired_token_handling(client):
    ...
    response = client.get(
        "/v1/admin/config", headers={"Authorization": f"Bearer {expired_token}"}
    )
    assert response.status_code == 401
...
def test_malformed_token_handling(client):
    ...
    response = client.get(
        "/v1/admin/config", headers={"Authorization": "Bearer malformed-token"}
    )
    assert response.status_code == 401

    response = client.get(
        "/v1/admin/config", headers={"Authorization": "NotBearer token"}
    )
    assert response.status_code == 401
...
def test_scope_case_sensitivity(client):
    ...
    response = client.get(
        "/v1/admin/config", headers={"Authorization": f"Bearer {token_lowercase}"}
    )
    assert response.status_code == 200
    ...
    response = client.get(
        "/v1/admin/config", headers={"Authorization": f"Bearer {token_uppercase}"}
    )

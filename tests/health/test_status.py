import os
import sys

sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), "..")))
from fastapi.testclient import TestClient


def test_status_endpoint(monkeypatch):
    """Test public status endpoints that don't require authentication."""
    os.environ["OLLAMA_URL"] = "http://x"
    os.environ["OLLAMA_MODEL"] = "llama3"
    os.environ["HOME_ASSISTANT_URL"] = "http://ha"
    os.environ["HOME_ASSISTANT_TOKEN"] = "token"
    from app import home_assistant, llama_integration, main, status

    async def fake_request(method, path, json=None, timeout=10.0):
        return {}

    async def fake_llama_status():
        return {"status": "healthy", "latency_ms": 1}

    monkeypatch.setattr(home_assistant, "startup_check", lambda: None)
    monkeypatch.setattr(llama_integration, "startup_check", lambda: None)
    monkeypatch.setattr(status, "_request", fake_request)
    monkeypatch.setattr(status, "llama_get_status", fake_llama_status)
    monkeypatch.setattr(
        status,
        "get_metrics",
        lambda: {
            "total": 1,
            "llama": 1,
            "gpt": 0,
            "fallback": 0,
            "session_count": 0,
            "transcribe_ms": 0,
            "transcribe_count": 0,
            "transcribe_errors": 0,
        },
    )

    client = TestClient(main.app)

    # Test public status endpoints that don't require authentication
    resp = client.get("/v1/status/integrations")
    assert resp.status_code == 200
    data = resp.json()
    assert "google" in data
    assert "spotify" in data

    resp = client.get("/v1/status/rate_limit")
    assert resp.status_code == 200
    data = resp.json()
    assert "window" in data
    assert "remaining" in data
